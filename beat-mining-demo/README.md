...
9. 按 `C` 可随时打开“设置”窗口；当前设置容器包含“节拍输入设置”，可拖动 `-300～+300 ms` 滑动条或使用左右方向键以 5 ms 为单位微调，点击“开始输入校准”（或按 `T`）可手动触发校准，`0` 恢复默认。校准会先短暂准备，再连续播放“滴”声；每次听到后按空格，共记录 4 次输入并给出建议补偿。补偿值保存在当前浏览器。
