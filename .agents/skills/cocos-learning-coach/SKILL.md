...
- **Assess:** Inspect the user's code and evidence, run available deterministic checks, collect runtime evidence once, ask only questions needed for uncovered objectives, and record the result.
...
- Keep runtime evidence collection separate from assessment questions. Do not repeatedly ask whether the game looks correct when the learner has already supplied the exercise's concrete preview results.
